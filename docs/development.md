...
The interface keeps financial amounts, deadlines, costs, restrictions, unavailable-data warnings, and action eligibility visible. Secondary information uses a shared native **Details** disclosure: **More filters** reveals quantity/payout/premium criteria, **Restrict to a wallet** reveals the optional holder restriction, and **Data sources & methodology** reveals observation timestamps and provenance. Collapsed summaries retain applied-filter counts and a configured holder address; closing a section does not clear its values. Offer creation groups token selection, terms, and funding, with one primary review action. Signing reviews keep all costs and restrictions visible while **Transaction accounts** reveals full addresses. Use disclosures for optional inputs and substantial technical context, not for a token name, balance, or single address. Disclosures support keyboard activation and retain their open state through background polling. This follows [progressive disclosure](https://www.nngroup.com/articles/progressive-disclosure/) and the [GOV.UK details guidance](https://design-system.service.gov.uk/components/details/) without hiding information needed to decide or authorize a transaction.
...
Local replica admission has no calendar cutoff: bootstrap uses the maximum supported signed timestamp for its review and expiry limits. Each offer still has its own acceptance and exercise deadlines. Bootstrap upgrades the original enabled, version-1 policies with matching review/expiry cutoffs through the program's policy-update instruction; repeated runs do not increment their version again. Disabled or deliberately revised policies are preserved. This rule belongs only to disposable local fixtures; mainnet admission and contract expiry checks are unchanged. To repair an older demo without resetting balances or agreements, rebuild and run bootstrap (`docker compose run --rm --build bootstrap`) in the same Compose project, then rebuild the application to receive frontend changes. Bootstrap prints its ledger identity; it must equal `genesisHash` from the running application's `/api/config`. A different value means the command targeted another local environment.
...
To try an independent demo while retaining another environment, stop it with `docker compose down` and start a separate Compose project with `docker compose -p volaryn-prestocks up --build`. Continue using `-p volaryn-prestocks` for that environment; its ledger and database volumes are independent.

To deliberately discard the disposable demonstration and obtain a fresh offer, use `docker compose down --volumes`, followed by `docker compose up --build`. This reset deletes the local ledger, manifest, projection, and server operation history; it is never performed by application startup. A changed SBF program requires a fresh disposable ledger: bootstrap rejects a retained ledger containing different program bytes rather than silently upgrading its contract. For native development, stop the launcher and remove the disposable `target/localnet` directory before rebuilding and restarting. A projection-only rebuild cannot replace deployed program bytes. A manifest-only schema update follows the narrower procedure under [external deployments and release checks](#external-deployments-and-release-checks).
...
The schema is defined by one initial migration. Before a retained-data release, development changes can replace that baseline and remove obsolete fields and interfaces; no legacy migration chain is maintained just for earlier implementation stages. Once a schema must preserve application-owned data, add and test explicit forward migrations instead. SQLx still rejects an applied checksum mismatch; startup never erases a database.

If an existing disposable local database has an older baseline, rebuild only its projection schema while preserving the Solana ledger and deployment manifest:

```sh
docker compose stop app
docker compose exec -T database psql -U postgres -d volaryn -v ON_ERROR_STOP=1 -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public AUTHORIZATION volaryn;'
docker compose up -d --build --wait --wait-timeout 300
```

This deliberately removes all application tables in the local `volaryn` database, including operation history. Use it only for a disposable database whose application history can be discarded. The application reapplies migrations and reconstructs agreement observations from the retained ledger; it cannot reconstruct every operation receipt. For the native demo, stop the launcher and remove only `target/localnet/postgres` before restarting; keep the ledger and deployment files. An ordinary additive migration, such as adding the activity table, does not require this reset.
...
| `backend/migrations/`         | Initial schema and necessary forward migrations, embedded in the executable.                     |
...
The public manifest uses schema 3. Local participant addresses and `fixtureVersion` are nested under `localnet`; external deployments omit that block. Both backend build features and frontend build mode reject the wrong deployment kind. The ordinary local Compose entry point is unchanged. Bootstrap accepts a schema-2 local manifest only when every recorded identity matches the expected ledger, program, assets and participants. It checks both the published manifest and any interrupted `.pending` file, then atomically publishes schema 3 after fixture verification succeeds. Existing balances are not topped up unless explicitly requested; the ledger and PostgreSQL data are retained. Backend and frontend still accept only schema 3.

Rebuild and run bootstrap when updating a retained demo; rebuilding only `app` leaves its generated manifest unchanged. For the `volaryn-prestocks` Compose project:

```sh
docker compose -p volaryn-prestocks run --rm --build bootstrap
docker compose -p volaryn-prestocks up -d --no-build --no-deps --force-recreate --wait --wait-timeout 300 app
```

A mismatch in identity or an unsupported schema fails without rewriting the manifest. A changed program build follows the separate disposable-ledger procedure above. The native full runner rehearses schema-2 manifest conversion while preserving used token balances, agreements and database history.
