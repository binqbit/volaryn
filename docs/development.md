...
Open `http://localhost:8080`. **Local test wallet** is the holder and **Local test writer** is the capital provider. Disconnect to change roles. The holder can activate the seeded offer; the writer can create additional offers from the Writer workspace without operational scripts. Set quantity, payout, premium, UTC deadlines, and an optional designated holder, then review and sign to reserve the payout. The holder uses Protection to match offers and choose whether to exercise. The writer can cancel unaccepted offers, reclaim unused reserves after expiry, and recover residual funds from terminal agreements. The wallet prompts for each signature. All identities, assets, and balances are disposable fixtures, including the six-decimal settlement currency labelled USDC.

Each review shows exact gross quantity, premium, payout, deadlines, current issuer fee and estimated net receipt, estimated SOL network fee, new-account rent, and issuer restrictions. Conditions are reread before signing. Amount inputs use exact decimal parsing; one raw-token unit is one million base units, unaffected by display scaling. Each transfer uses one selected token account. The wallet lists accounts separately, marks frozen holdings, and shows delivered settlement accounts under the writer's ownership. A total across accounts does not authorize delivery from an account with insufficient funds. Indexed lists can lag a newly finalized offer until the next discovery pass; its direct agreement URL can read chain state immediately.
...
This deliberately removes all application tables in the local `volaryn` database. Use it only while that database contains the rebuildable projection, deployment identity, and checkpoint. The application reapplies the baseline and reconstructs observations from the retained ledger. For the native demo, stop the launcher and remove only `target/localnet/postgres` before restarting; keep the ledger and deployment files.
...
Playwright rejects a wallet signature, blocks a changed network, activates through real wallet signing, reopens the tab and reconnects while confirmation is pending, and exercises without the writer. It checks finalized agreement state, gross delivery, issuer fees, full USDC payout, reserve depletion, and writer ownership of the delivered account. Writer scenarios create offers through the browser, cancel and recover terminal funds, match exact terms with a separate holder, verify delivered-account ownership, and wait for real validator expiry before reclaiming. Separate interface scenarios verify explicit connection, balance removal on disconnect, wallet loading/empty/error states, split-account delivery limits, ownership-aware presentation, and wallet changes during preparation. Recovery checks restart PostgreSQL and the validator/application, repeat bootstrap, and rebuild the chain projection while retaining identical financial state. Test cleanup removes only that run's project and volumes. Logs, browser traces on failure, screenshots, and rendered configuration remain under `artifacts/tests/localnet.*`.
