...
To try schema 2 while retaining the previous demo, stop it with `docker compose down` and start a separate Compose project with `docker compose -p volaryn-prestocks up --build`. Continue using `-p volaryn-prestocks` for that environment; its ledger and database volumes are independent.

To deliberately discard the disposable demonstration and obtain a fresh offer, use `docker compose down --volumes`, followed by `docker compose up --build`. This reset deletes the local ledger, manifest, projection, and server operation history; it is never performed by application startup. Deployment schema 2 replaces the single-mint fixture with the PreStocks asset list; a schema-1 demo requires this explicit reset and rebuilt app/bootstrap images. A changed SBF program also requires a fresh disposable ledger: bootstrap rejects a retained ledger containing different program bytes rather than silently upgrading its contract. For native development, stop the launcher and remove the disposable `target/localnet` directory before rebuilding and restarting. A projection-only rebuild cannot replace deployed program bytes.
