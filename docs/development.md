...
The initial migration defines chain projections; a separate additive migration defines durable activity receipts. Keep applied migrations immutable when application history must be retained, and test explicit forward migrations. Baseline replacement is limited to disposable development databases whose off-chain history may be discarded. SQLx still rejects an applied checksum mismatch; startup never erases a database.
