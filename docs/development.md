...
Wallet and agreement observations poll three seconds after each completed read; official context polls every thirty seconds. Successful finalized or reconciled operations also trigger an immediate refresh of balances, activity, offers and portfolio queries. A background read preserves the displayed result, form values, focus, and controls instead of adding loading rows or hiding an empty list. Changed data appears when the response succeeds. Initial reads still show loading, and failures retain their warning and pause dependent financial actions throughout retries until recovery. New wallets and queries never reuse another identity's observations.
...
The validator loads the compiled upgradeable program and pinned token programs into a new genesis. Bootstrap verifies the program hash, records the genesis and fixture identity in a public manifest, then initializes protocol configuration, the settlement mint, eight PreStocks replica mints, their policies, participant accounts and balances through transactions. It creates **no offers or agreements**. Both test wallets start with **10,000 available test USDC and 100 unscaled units of each of the eight replicas**; Explore offers and both portfolios are empty. Create a sell request or buy offer through the form before switching wallets to accept it. These interactive balances are independent of the smaller contract-test balances. An existing ledger is resumed; bootstrap reuses compatible accounts and preserves user-created agreements and spent balances. A partially completed bootstrap resumes through its pending manifest. Incompatible identity fails explicitly, without resetting or converting retained state.

On a fresh ledger, bootstrap waits for transaction finalization before using dependent accounts. Independent participant funding and the eight asset initialization sequences run concurrently; USDC and protocol configuration retain their dependency order. Initialization can take several minutes after the validator becomes healthy. Compose waits for this job to exit successfully before starting the application. Follow `docker compose logs -f bootstrap` for the current operation, a waiting message every ten seconds during transactions, and completion times. Reusing an initialized ledger skips completed initialization transactions and adds no offers. Bootstrap source changes require rebuilding its image; `--no-build` continues to use the previously packaged code.
...
No environment file is required. By default, background workers check known agreements and submitted operation receipts every second. New finalized addresses are discovered on the first worker pass after a two-second discovery interval, so discovery passes normally occur two to three seconds apart at the default one-second cadence, plus RPC and database work. The delay for an individual creation depends on when it becomes finalized within that cycle. Successful finalized receipts also refresh their known agreement directly before becoming completed history; they do not wait for full discovery. Failed refreshes leave receipts eligible for retry. Reads and writes retain finalized commitment and monotonic slot checks.

The browser checks transaction status one second after each observation completes and refreshes visible lists and balances three seconds after each read. A successful finalized or reconciled operation triggers an additional immediate refresh. These intervals describe scheduling, not a maximum end-to-end latency: Solana finalization, network latency and RPC availability still apply. Pending and confirmed transactions remain visibly distinct from finalized agreements; reducing a polling interval never skips finality.

[.env.example](../.env.example) contains only two optional settings:

| Variable                                | Default | Accepted range    | Effect                                                  |
| --------------------------------------- | ------- | ----------------- | ------------------------------------------------------- |
| `VOLARYN_INDEX_POLL_INTERVAL_SECS`      | `1`     | `1`–`10` seconds  | Cadence of agreement and operation reconciliation.      |
| `VOLARYN_INDEX_DISCOVERY_INTERVAL_SECS` | `2`     | `1`–`300` seconds | Minimum interval between full address-discovery passes. |

For local Compose, copy `.env.example` to `.env`, change only the needed values, and recreate the application with `docker compose up -d app`. For a live deployment, put overrides in `deployment/release.env`, the explicit Compose environment file. Native processes read exported environment variables; they do not load `.env` automatically. Invalid values fail at startup. Lower intervals increase RPC load; the configured discovery deadline is checked at worker ticks and does not create a second scheduler. Isolated native and Compose test runners explicitly use the default cadence so operator overrides cannot change test timing.
...
The runner builds the contract and application with their checks, then starts a unique Compose project with its own ledger, deployment manifest, and PostgreSQL volume. Backend integration tests run against that server with separate databases before the application starts. The rendered configuration is checked for inherited host ports and external volumes. Automated native and Compose runners invoke `tools/localnet/browser-fixtures.ts`, which first verifies zero on-chain agreements, empty agreement and offer lists, and the full starting balances in both wallets. Only then does it create two funded offers needed by the browser scenarios. Ordinary Compose and native development startup never invoke this test-only helper. A loopback forwarder inside the browser-test container supplies a secure browser origin without publishing ports or requiring WebSockets. The default demo can remain running.
...
`tests/fixtures/recipe.json` defines shared identities, quantities, and localnet token behavior. The Rust fixture verifies its test vocabulary against that recipe. Bootstrap consumes the identities and token behavior; the explicit test-only offer helper and browser assertions consume its agreement terms. Normal startup creates no agreements. Localnet uses the validator clock for offer deadlines; LiteSVM uses controlled time for exact boundary scenarios.
