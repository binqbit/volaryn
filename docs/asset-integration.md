...
The tests cover missing and changed fields, exact numbers, unverified units, unsupported accounts, issuer-authority drift, lifecycle boundaries, epoch fee selection, display scaling, timeouts, rate limits, bounded retries, cache recovery, and coalesced reads. Browser tests use controlled responses to verify explicit mainnet context, missing/stale values, mobile layout, and the absence of financial actions. Both holder-origin sell requests and writer-origin buy offers use the same mint admission and delivery rules. Local holder and writer journeys use disposable fixtures, including premium escrow, payout funding, and exercise after either acceptance path.
