...
The index and receipt workers run within the application. Known open/active agreements refresh every second by default. Full finalized address discovery runs on the first pass after a two-second interval; successful finalized operation receipts also trigger a targeted refresh without waiting for discovery. Failed targeted refreshes remain retryable. See [refresh timing](development.md#refresh-timing) for the two optional controls and the distinction between scheduling and network finality. Index writes and slot checkpoints are transactional, and older observations cannot overwrite newer ones. Direct agreement reads can fall back to the chain when the projection is unavailable.
