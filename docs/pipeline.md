...
**Working result:** one local startup launches a small browser application connected to the tested contract with an empty offer market. A participant creates an offer, the opposite wallet accepts it, and the holder exercises using disposable test assets.
...
- Load the program into the local validator at ledger genesis. Add an idempotent bootstrap job that verifies its identity, initializes protocol and asset policies, and prepares two test wallets with 10,000 available USDC and 100 unscaled units of every supported replica. Startup creates no agreements; both portfolios are empty on a fresh ledger.
- Expose network identity, supported fixture positions, user-created offers, agreement terms, and transaction outcomes. Store a minimal rebuildable projection of chain state, applying the current initial database schema before readiness, and reconcile it from authoritative accounts on startup and after transactions.
...
- Add the full test entry point with an isolated ledger, database, and browser environment per run. Assert empty startup and untouched wallet balances before explicitly creating two funded offers with a test-only helper. Use the real compiled program and disposable wallet signatures; share the automated fixtures and runner between native and Compose checks, with failure artifacts and scoped cleanup. Keep test-offer provisioning out of ordinary startup.

**Verification**

Start from a clean checkout using the documented local entry point without mandatory environment variables. Verify empty discovery and portfolios, then create an offer, complete acceptance and exercise through the browser, and verify balances and agreement state on the ledger. Reject wallet/network mismatch and wallet-signature rejection without showing success. Restart the environment while retaining user-created agreements and spent balances, without creating new offers or resetting state. Check that the generated client executes the same contract interface tested in Stage 1. Repeat the automated run from fresh state and alongside a manual demo to verify isolation, useful failure evidence, and cleanup that leaves the demo untouched.
