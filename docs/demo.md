...
Connect **Test Wallet 2** and open **Create offer**. Choose a token, set the quantity, payout, premium and future UTC deadlines, and leave the optional holder restriction empty or set it to Test Wallet 1. **Review funded offer → Confirm and sign** reserves the payout. Once finalized, copy its page address, disconnect and use Test Wallet 1 to activate it.

An unaccepted offer can be cancelled by its writer. An activated offer cannot: leave it unused until its protection expiry, then use the writer's **Reclaim expired reserve** action. The writer recovers the reserved payout and keeps the premium; the holder keeps their remaining tokens and receives no payout. Neither expiry nor a falling price automatically submits a transaction.
...
For reproducible verification, `./tools/test full` runs the isolated Compose journey and recovery checks. The [development guide](development.md#application-checks) also provides the native localnet runner, which demonstrates settlement with the application stopped. Real-asset hosting follows the separate [deployment guide](deployment.md).
