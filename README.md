...
2. Propose your own sell request: choose the fixed token quantity, USDC payout, premium, and expiry, then escrow the premium. A capital provider accepts by reserving the full payout and receiving that premium atomically.
3. Alternatively, accept a writer's buy offer whose payout is already reserved, paying its premium. Either path activates the same protection; the underlying stays in the holder's wallet. An unmatched sell request has no exercise right yet.
...
Open `http://localhost:8080` and choose **Connect wallet**. Select **Test Wallet 1** for the holder journey or **Test Wallet 2** to fund and manage offers. Disconnect before switching roles. Every financial action opens a terms review before wallet signing. For Test Wallet 1 and Test Wallet 2, **Confirm and sign** signs and submits after rechecking the reviewed conditions, with no additional confirmation dialog. **Back** or Escape closes the review without signing. External wallets retain their own signing approval. The local environment provides eight clearly labelled PreStocks replicas, including OpenAI, SpaceX and Anthropic, with issuer-fee simulations and real local-chain settlement. A fresh environment has **no offers or agreements** and both portfolios are empty. Each test wallet has **10,000 available test USDC and 100 unscaled units of every supported replica**. Create a request or buy offer before switching wallets to accept it. Existing demos can be refilled with the explicit [top-up command](docs/development.md#run-the-local-application). No environment file or external account is required. Restarting preserves the ledger, database, balances, and user-created agreements; startup never clears them or creates offers. Native development can use `npm run dev:localnet`; its pinned prerequisites and build commands are in the development guide.
...
- **Explore offers** (`/offers`) searches PreStocks by name, ticker or mint and separates **All offers**, **Sell requests**, and **Buy offers**. Sell requests originate from token holders and escrow a premium; buy offers originate from capital providers and reserve the payout. Type badges remain visible on cards. Token, exact quantity, payout, premium, and counterparty filters apply before pagination. Open a card to review terms and accept from the opposite role.
- **Create offer** (`/offers/new`) selects a PreStocks replica, shows your token holdings and available funding USDC, and offers editable dates within the asset's policy limits. Enter a premium in USDC or as a percentage of the payout; the form explains its example defaults and preserves manual edits. The default **Request protection** mode escrows the premium; **Provide protection** reserves the full payout. Switching the side preserves entered terms. See [suggested offer terms](docs/offer-lifecycle.md#suggested-offer-terms). Activation and exercise show the required amount and selected-account balance beside their account selectors.
- **My portfolio** (`/portfolio`) opens **All**, combining agreements held or written by the connected wallet, including completed agreements. **My protection** (`/portfolio/protection`) and **My offers** (`/portfolio/written`) filter by role; the independent status filter retains its selection across role changes, reloads, and pagination. Open an agreement to cancel or reclaim eligible reserves.
- **Activity** (`/portfolio/activity`) retains pending, completed, failed, and interrupted operations. Pending creations appear in **All** and the creator's role view before finalization; holder requests appear under **My protection**, capital offers under **My offers**. The wallet reconnects after reload when its permission remains valid; **Disconnect** clears that preference. See the [offer lifecycle](docs/offer-lifecycle.md) for transaction states, persistence, and recovery.
- **Your wallet** shows available USDC first, followed by compact PreStocks cards in a bounded, scrollable list. On desktop, the panel stays beside the content with identity and USDC above the list when the viewport has enough room. Names, exact balances, frozen amounts, and full account addresses stay on each card without extra disclosures. Multiple accounts show their individual balances, while a single account does not repeat the total. **View wallet** in the header opens this panel from any page, including on mobile; the header does not repeat balances. Frozen funds are excluded from available USDC. Exercise delivers from one selected account; separate balances are not combined.
